...
import { Repeat, Plus, X, Calendar, DollarSign, TrendingUp, Link as LinkIcon } from "lucide-react";
...
export default function SubscriptionsPage() {
  const { subscriptions, setSubscriptions, clients, projects } = useData();
  ...
  // Handle prefill from Projects/Client "Start Subscription"
  ...
  const addSub = () => {
    ...
    const project = form.projectId ? projects.find((p: { id: string }) => p.id === form.projectId) : null;
    const sub = {
      id: "sub_" + Date.now(),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        {subscriptions.map((sub: { id: string; clientName: string; plan: string; amount: number; interval: string; status: string; startDate: string; nextPayment: string; projectId?: string }) => {
